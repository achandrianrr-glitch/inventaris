    ...
    public function index(Request $request): Response
    {
        ...
        if ($filters['search'] !== '') {
            ...
            $q->where(function (Builder $x) use ($s) {
                $x->where('code', 'like', "%{$s}%")
                    ->orWhereHas('borrower', fn($bq) => $bq->where('name', 'like', "%{$s}%")->orWhere('id_number', 'like', "%{$s}%"))
        ...
        // indikator terlambat (tanpa cron dulu): tampilkan flag overdue saat status masih borrowed & due lewat
    ...
    public function store(BorrowingStoreRequest $request)
    {
        ...
        return DB::transaction(function () use ($data, $adminId) {
            ...
            if (($data['borrow_type'] ?? '') === 'lesson') {
                // Untuk jam pelajaran: due di hari yang sama jam 23:59 (wajib balik hari itu)
                $returnDue = Carbon::parse($data['borrow_date'])->endOfDay();
            } else {
                // daily: dari input return_due_date, due jam 23:59
