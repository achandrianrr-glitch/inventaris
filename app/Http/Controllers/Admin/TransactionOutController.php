...
class TransactionOutController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $q = Transaction::query()
            ->where('type', 'out')
            ->with([
                'item:id,code,name', // kalau mau tampilkan item trashed juga: ubah relasi item() di Transaction jadi withTrashed
                'admin:id,name',
            ])
            ->select([
                'id',
        ...
        return Inertia::render('Admin/Transactions/Out/Index', [
            ...
            'filters' => $filters,
            'options' => [
                // ✅ hanya tampilkan item yang layak ditransaksikan (bukan trashed, bukan inactive/service)
                'items' => Item::query()
    ...
    public function store(TransactionOutStoreRequest $request)
    {
