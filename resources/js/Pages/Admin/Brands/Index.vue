...
function openEdit(b) {
    mode.value = "edit";
    editing.value = b;
    form.value = { name: b.name ?? "", status: b.status ?? "active" };
    showModal.value = true;
}
...
function submit() {
    if (mode.value === "create") {
        router.post("/admin/brands", form.value, {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    } else {
        router.put(`/admin/brands/${editing.value.id}`, form.value, {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    }
...
function softDelete(b) {
    if (!confirm(`Hapus merek "${b.name}"? (soft delete)`)) return;
    router.delete(`/admin/brands/${b.id}`, { preserveScroll: true });
}

function restore(b) {
    router.patch(`/admin/brands/${b.id}/restore`, {}, { preserveScroll: true });
}
</script>
...
<template>
    <div class="container-fluid">
        <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between mb-3">
            <div>
                <h5 class="mb-1">Merek</h5>
                <div class="text-muted small">CRUD merek + jumlah barang per merek + soft delete</div>
            </div>
        ...
        <div class="panel p-3 mb-3">
            <div class="row g-2 align-items-end">
                <div class="col-12 col-md-6 col-xl-6">
                    ...
                    <input v-model="search" type="text" class="form-control" placeholder="Cari nama merek..." />
                </div>
        ...
        <div class="panel d-none d-md-block">
            <div class="table-responsive">
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        <tr v-for="b in brands.data" :key="b.id" :class="b.deleted_at ? 'row-trashed' : ''">
                            ...
                            <td>
                                <span class="badge"
                                    :class="b.status === 'active' ? 'text-bg-success' : 'text-bg-warning'">
                                    {{ b.status }}
                                </span>
                            </td>
                            <td class="text-end">
                                <span class="badge text-bg-light">{{ b.items_count ?? 0 }}</span>
                            </td>
                            <td class="text-muted small">{{ new Date(b.updated_at).toLocaleString() }}</td>
                            <td class="text-end">
                                <div class="btn-group">
                        ...
                        <tr v-if="brands.data.length === 0">
                            <td colspan="5" class="text-muted p-3">Tidak ada data merek.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="p-3 d-flex justify-content-between align-items-center">
                <div class="text-muted small">
                    Page {{ brands.current_page }} / {{ brands.last_page }} • Total {{ brands.total }}
                </div>
                <div class="d-flex flex-wrap gap-1">
                    <button v-for="(l, idx) in brands.links" :key="idx" class="btn btn-sm"
        ...
        <!-- mobile cards -->
        <div class="d-md-none">
            <div v-for="b in brands.data" :key="b.id" class="mini-card" :class="b.deleted_at ? 'row-trashed' : ''">
                <div class="d-flex justify-content-between align-items-start">
                    <div class="me-2">
                        ...
                        <div class="mt-2 d-flex gap-2 align-items-center">
                            <span class="badge" :class="b.status === 'active' ? 'text-bg-success' : 'text-bg-warning'">
                                {{ b.status }}
                            </span>
                            <span class="text-muted small">Barang: {{ b.items_count ?? 0 }}</span>
                        </div>
                        <div class="text-muted small mt-1">{{ new Date(b.updated_at).toLocaleString() }}</div>
                    </div>
            ...
            <div v-if="brands.data.length === 0" class="text-muted p-2">Tidak ada data merek.</div>

            <div class="mt-2 d-flex justify-content-between align-items-center">
                <div class="text-muted small">Total {{ brands.total }}</div>
                <div class="d-flex flex-wrap gap-1">
                    <button v-for="(l, idx) in brands.links" :key="idx" class="btn btn-sm"
                        :class="l.active ? 'btn-primary' : 'btn-outline-secondary'" :disabled="!l.url" v-html="l.label"
                        @click="l.url && router.get(l.url, {}, { preserveState: true, preserveScroll: true })" />
            ...
            <div v-if="showModal" class="modal-backdrop-custom" @click.self="closeModal">
                <div class="modal-card">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <div class="fw-semibold">
                            {{ mode === 'create' ? 'Tambah Merek' : 'Edit Merek' }}
                        </div>
                    ...
                    <div class="mb-2">
                        ...
                        <input v-model="form.name" type="text" class="form-control"
                            placeholder="Contoh: Canon / Lenovo" />
